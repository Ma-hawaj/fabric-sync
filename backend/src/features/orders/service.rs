...
use super::{repository, types::Order};
...
/// received, settles the invoice's remaining balance in full.
pub async fn receive_order(state: &AppState, order_id: Uuid) -> Result<Order, AppError> {
    ...
    if repository::invoice_fully_received(&mut tx, invoice_id).await? {
        repository::mark_invoice_paid(&mut tx, invoice_id).await?;
    }
