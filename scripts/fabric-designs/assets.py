...
def main() -> int:
    ...
    # section, slug, label, rel path, width, height
    embedded: list[tuple[str, str, str, str, int, int]] = []
    ...
    for section, options in catalog.items():
        ...
        for opt in options:
            ...
            embedded.append((section, opt["slug"], opt["label"], rel, width, height))
            total += 1
...
def _write_backend_module(embedded: list[tuple[str, str, str, str, int, int]]) -> None:
    entries = [
        ...
        for section, slug, label, rel, width, height in embedded
    ]
