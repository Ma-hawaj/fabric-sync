...
- `assets.py` caps every raster at 512px and encodes it as WebP, then writes **two trees plus two generated modules**, all of which must stay in sync by regenerating:
  ...
  - `backend/assets/designs/` + the **generated** `backend/src/features/invoices/designs.rs` (`DesignAsset { section, slug, label, bytes }` inside one `DESIGNS` static), so a no-network print renderer can inline them as `data:image/webp;base64,` URIs.
...
- The printed invoice shows each choice as a chip: `document.rs::line_designs` walks the five slots in fixed order (Thobe / Collar / Sleeve / Pocket / Patti), looks the stored value up against `DESIGNS`, and renders an image chip when it matches and a text-only chip otherwise. The per-slot values reach it via `InvoiceDetailLine.design_values` (`#[serde(skip)]`, so the REST payload keeps the pre-joined `detail` string and no base64 bloat). The free-text note prints separately from `line_notes`.
