...
- **Auth**: `auth.rs` implements OAuth2/OIDC token introspection (`TokenIntrospection::discover` does OIDC discovery or uses `OAUTH_INTROSPECTION_URL` directly). `require_role(minimum: Role)` builds an axum middleware that validates the bearer token, checks the resolved role against `minimum`, and inserts `AuthenticatedUser` (roles and all) as a request extension. See "Roles and permissions" below for the role model and how it's wired into `app.rs`. A missing/invalid bearer token returns 401 via `AppError::Unauthorized`; an authenticated caller without the required role returns 403 via `AppError::Forbidden`.
- **Errors**: all fallible backend code returns `Result<_, AppError>` (`error.rs`), a single enum with one `IntoResponse` impl. Variants: `Auth` (500, boot-time OIDC-discovery failure only), `Unauthorized` (401, missing/invalid token), `Forbidden` (403, authenticated but missing the required role), `Io`, `Sqlx`, `Migration` (all 500), `NotFound` (404), `Conflict` (409), `BadRequest` (400). Add new variants there rather than converting to strings/status codes ad hoc in handlers.
...
- **Auth is real OIDC** (Authorization Code + PKCE via `react-oidc-context`/`oidc-client-ts`, generic/OIDC-standard — Zitadel locally, but not hardcoded to it). `src/lib/oidc.ts` holds the `UserManager` (config from `VITE_OIDC_*` env vars, token in `sessionStorage`) and `getAccessToken()`. `src/lib/auth.tsx`'s `AuthProvider` wraps `react-oidc-context`'s provider and adapts its state to the app's `AuthState` shape (`isAuthenticated`, `isLoading`, `user`, `signIn`, `signOut`), `user.roles` included — see "Roles and permissions" below. `useAuth()` is unchanged as the consumption point. `src/routes/_authenticated.tsx` guards routes via `beforeLoad`, calling `context.auth.signIn(location.href)` to redirect straight to the IdP's hosted login when unauthenticated — there is no local `/login` page (auto-redirect UX); it does not check roles. `main.tsx` gates mounting `RouterProvider` on `auth.isLoading` so the redirect check never fires before `react-oidc-context` finishes resolving the session on boot.
...
Three roles, in a hierarchy — `admin` ⊇ `staff` ⊇ `viewer` (a higher role satisfies a lower role's gate) — sourced from Zitadel's project-role assertion claim, `urn:zitadel:iam:org:project:roles` (a JSON object `{ "<roleKey>": { "<orgId>": "<orgName>" } }`; the role keys, not the values, are what matter here). This requires the roles to exist on the Zitadel project with exactly these (lowercase, case-sensitive) keys and "Assert Roles on Authentication" enabled — operator setup done in the Zitadel console, not in this repo.

**Backend**: `backend/src/auth.rs`'s `Role` enum (`Viewer < Staff < Admin`, so `#[derive(PartialOrd, Ord)]` gives the hierarchy) is parsed off the RFC 7662 introspection response's extra fields (`ZitadelExtraFields`, since the `oauth2` crate's `BasicClient` otherwise fixes extra fields to `EmptyExtraTokenFields` and silently drops the claim). `AuthenticatedUser::roles()`/`has_role(minimum)` expose it. Every currently-guarded feature router in `app.rs` requires the same baseline role for now — `.route_layer(middleware::from_fn_with_state(state.clone(), auth::require_role(Role::Staff)))` — one uniform gate rather than per-route/per-method roles; only `health` is unguarded. Handlers already extract `Extension<AuthenticatedUser>` (see e.g. `customers/handlers.rs`) — reach for `user.has_role(...)` there to add a finer-grained check for a specific route without touching `app.rs`.

**Frontend**: `src/lib/auth.tsx` derives `AuthUser.roles: Role[]` from the ID token profile's `urn:zitadel:iam:org:project:roles` claim (`react-oidc-context`'s `user.profile`, which carries it once Zitadel asserts it — no special OIDC scope needed) and exports `hasRole(user, minimum)`, mirroring the backend's hierarchy. `src/components/role-guard.tsx`'s `<RoleGuard role="admin">…</RoleGuard>` (or `role={['admin', 'staff']}` for any-of, with an optional `fallback`) wraps UI that should only render for a given role — there is no route-level (`beforeLoad`) role gating yet, only this component-level one.
