...
Every `GET` list route above except `GET /users` is served by the shared list layer and returns `{data, page, perPage, total, pageCount}` — never a bare array. `GET /users` instead returns a bare array from `AuthentikUserDirectory` (see below), with no database or `ListSpec` behind it.
