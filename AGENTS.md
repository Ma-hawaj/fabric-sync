...
- Protected pages live under `src/routes/_authenticated/`: customers, inventory, invoices, and locations (each an `index` + `new` pair; locations also has `$locationId/edit`), plus flat `orders.tsx`, `users.tsx`, and `dashboard.tsx`. Only `src/routes/index.tsx` is outside that guard. Each route file sets `staticData: { title }`, which is the entire breadcrumb integration — `components/breadcrumbs.tsx` reads titles off matches generically, so there's no route-segment-to-label map to update.
...
| `invoices` | `GET /invoices`, `POST /invoices`, `GET /invoices/:id`, `GET /invoices/:id/document`, `GET /invoices/:id/ready-card`, `POST /invoices/:id/receive` |
...
| `users` | `GET /users` |
| `whatsapp` | `POST /whatsapp/invoices/:id/messages` |
...
- **`backend/templates/invoice.html` is the file to edit to restyle it** — minijinja, with the available context documented in a comment at the top. It is `include_str!`-embedded by default; set `INVOICE_TEMPLATE_DIR` to load from disk instead, and design changes need no rebuild. The same section header applies to the WhatsApp ready card, a second document in the same module: `backend/templates/invoice-ready-card.html`, rendered by `document.rs::render_ready_card`, served at `GET /invoices/:id/ready-card`. As its name says it is a compact "your order is ready for collection" notice (banner + headline + a summary of orders ready/total/paid/balance + company footer), used instead of the invoice image when the last order on an invoice is production-complete.
...
## WhatsApp (Cloud API — the backend sends, the frontend captures)

The invoice image is sent for real, via Meta's **WhatsApp Business Cloud API**: the frontend rasterizes the invoice document to a PNG (a browser's job), posts it to the backend, and the backend uploads it to `/media` and sends it to the customer's number. The image lands in the customer's chat — no operator handoff, no wa.me paste. The feature is three opt-in entry points that all land on the same dialog:

- **Backend `features/whatsapp/`** — mirrors the users feature's external-API shape. `WhatsAppClient` (`service.rs`) holds the Graph credentials from `Config` (`WHATSAPP_PHONE_NUMBER_ID`, `WHATSAPP_ACCESS_TOKEN`, optional `WHATSAPP_API_VERSION` defaulting to `v21.0`) and exposes `send_invoice_image`: normalize the recipient, decode the base64 PNG (capped at the Cloud API's 5 MB), upload it to `/{phone_number_id}/media` as multipart, then send `POST /{phone_number_id}/messages` as an `image` message with a caption. The caption is built server-side from `invoices::service::get_invoice` + `InvoiceBranding` — no display text is trusted from the client. The route is `POST /whatsapp/invoices/:id/messages` with a raised body limit (`DefaultBodyLimit::max(8 MB)`) because the base64 payload exceeds axum's 2 MB default. The pure seams are `normalize_recipient`, `decode_invoice_image`, `normalize_mime`, and `invoice_caption` (unit-tested). WhatsApp's own `error.message` is passed straight through as `AppError::WhatsApp` (502) so the UI can show the real reason — unverified recipient number, and above all: **a plain image send only works inside the 24-hour customer-service window** (i.e. the customer must have messaged this number recently). Outside it WhatsApp rejects the send; an approved template could carry a *fixed* image header but never a per-invoice image, so no template path is wired. `WhatsAppClient::from_config` returns `None` when the env vars are unset — the endpoint then answers 400, not a boot failure.
- **`src/components/send-invoice-whatsapp-dialog.tsx`** — the single "send the invoice to the customer" component. It fetches `GET /invoices/:id` (via `useInvoice`, key `['invoices', id]`) for the INV number, totals, and recipient phones (the retail `buyer`, or deduplicated per-line `customer`s), captures the document to a PNG, and calls `useSendInvoiceWhatsApp` (key-less mutation hook, `POST /whatsapp/invoices/:id/messages` with `{ to, mediaBase64, mimeType }`, base64 via `FileReader.readAsDataURL`). When `kind === 'ready'` (the last order is production-complete) the captured image is the **ready-for-collection card** (`GET /invoices/:id/ready-card`) rather than the invoice document. The captured PNG is shown as an inline preview (a "Preview as PDF" button runs the print path — the send is only enabled once the preview is ready), and the same blob is what gets sent. A recipient picker appears only when an invoice covers several people; with no phone on file, Send is disabled. On success the dialog closes; on failure it stays open and shows the backend's message (which is why `ApiError` in `src/lib/api.ts` prefers the plain-text server body over axios's generic message).
- **`src/features/invoices/lib/capture-invoice-image.ts`** — `captureInvoiceImagePng(id)` and `captureReadyCardImagePng(id)` fetch their document (`GET /invoices/:id/document`, `GET /invoices/:id/ready-card`) as text (same auth path as printing) and rasterize the self-contained HTML inside an offscreen iframe via **`html-to-image`** (`toBlob`, `pixelRatio: 2`) — one shared `captureDocumentPng(path)` under both. Because the documents are all inline styles/SVG/`data:` URIs there is no cross-origin taint. If this ever fails on a machine, the design-wide fallback is printing to PDF — don't reach for a server-side renderer.
- **`src/lib/whatsapp.ts`** — now just `normalizePhoneNumber` (digits only, the shape both the WhatsApp API and a `wa.me` href want). Sending and the caption live in the backend, so there's no message-construction or delivery code left in the frontend.
- **`src/features/orders/lib/invoice-completion.ts`** — `isInvoiceFullyReady(invoiceId, queryClient)` fetches `GET /orders` filtered server-side on `invoiceId` and returns true when every order is production-complete (`currentStage === null`) or already `received`.

The three triggers: (1) the invoice form's **"Save & Send via WhatsApp"** submit button (a `sendAfterSave` ref beside `exportAfterSave`, navigating to `/invoices` once the dialog closes), (2) completing the **last outstanding stage of the last outstanding order** on an invoice — `order-tracking-panel.tsx`'s `StageRow` fires `onOrderReady` on the `currentStage: non-null → null` transition and `order-detail.tsx` offers the dialog only after `isInvoiceFullyReady` passes, and (3) the **"Send via WhatsApp"** resend button on `invoice-detail.tsx`. Adding a resend anywhere new means rendering the same dialog — there is no other message-construction code to keep in step.
