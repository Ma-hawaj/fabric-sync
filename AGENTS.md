...
**Payments are a ledger, not two columns.** `invoice_payments` holds every payment (advances at creation, per-pickup payments when orders are collected one by one, till payments via `POST /invoices/:id/payments`); `invoices` stores no money state at all. `amount_paid` is the ledger sum, `payment_status` derives from the balance (`paid`/`partial`/`unpaid`), and `payment_method` is the latest payment's method — all worked out in the queries via lateral joins, with `service::payment_summary`/`validate_payment_amount` as the pure, tested seam. `POST /orders/:id/receive` takes `{amount, paymentType}` (zero collects with no money) and refuses already-received orders so a retry can't double-charge; `POST /invoices/:id/receive` marks every pending order and settles the remainder in one payment (no money when the balance is already zero). An invoice is settled only once every order is received *and* the balance reaches zero; overpaying the balance is a 400, and concurrent pickups serialize on a `FOR UPDATE` invoice lock. Editing an invoice is refused once any payment exists (the ledger can't be rebuilt like the old header overwrite could).
...
| `invoices` | `GET /invoices`, `POST /invoices`, `GET /invoices/:id`, `GET /invoices/:id/document`, `POST /invoices/:id/receive`, `POST /invoices/:id/payments` |
...
**Line prices are VAT-inclusive; 10% VAT is extracted from the discounted gross, floored at zero.** The rate is written twice — `VAT_RATE` in `backend/src/features/invoices/service.rs` (authoritative; the stored total) and `VAT_RATE` in `frontend/src/features/invoices/lib/invoice-pricing.ts` (`computeInvoiceTotals`, the on-screen running total behind `invoice-summary.tsx`). Change one and you must change the other. `service::breakdown` splits the discounted gross into net (`gross / 1.1`, rounded) plus VAT as the remainder, so taxable + VAT always adds back to the gross on the page. Currency is single-valued by design: `CURRENCY = 'BHD'` in `src/lib/currency.ts`, and `CURRENCY`/`CURRENCY_DECIMALS` in `invoices/document.rs` for the printed document (BHD is a three-decimal currency, so amounts print as fils even though they are stored `NUMERIC(10, 2)`).
