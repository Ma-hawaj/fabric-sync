...
Local infra (Postgres + Authentik IdP) via `docker-compose.yml` at repo root:
...
Postgres is pinned to **18+** (`POSTGRES_IMAGE` in `.env`/`.env.example`, and `postgres:18-alpine` in `.github/workflows/backend.yml`) — this is a hard requirement, not just the latest-available tag: schema id defaults use the native `uuidv7()` function, which only exists in Postgres 18+. The compose volume mount is `postgres-data:/var/lib/postgresql` (the whole data dir, not `.../data`) because the official image changed its on-disk layout starting in 18 to support `pg_upgrade --link`; mounting at the old `.../data` path makes 18+ images refuse to start. Bumping the Postgres image tag again always needs a fresh volume (`docker compose down && docker volume rm fabric-sync_postgres-data`) since minor/major version jumps aren't in-place compatible — remember this also resets local Authentik state, since it shares the same Postgres server.
...
**`users` is backed by Authentik's real user directory**, not a table — `features/users/service.rs::list_users` delegates to `features/users/authentik.rs::AuthentikUserDirectory`, so there's still no `repository.rs`. `id` is a plain `String` rather than a `Uuid` because it holds an Authentik user UUID, not an id this app generates. It's `pub(crate)` (see `users/mod.rs`) so `orders/service.rs::set_assignee` can resolve an assignee's display name against it directly — the same cross-feature-call pattern `invoices` already uses against `customers`/`products`.

`AuthentikUserDirectory` calls Authentik's own core Users API (`GET {base}/api/v3/core/users/`) — not part of the OIDC standard, so unlike `auth::TokenIntrospection` it is unapologetically Authentik-specific. It authenticates with a static API token issued to a service account (`AUTHENTIK_API_TOKEN` / `AUTHENTIK_BASE_URL`), deliberately separate from `OAUTH_CLIENT_ID`/`OAUTH_CLIENT_SECRET`: the latter authenticates the confidential OAuth2 provider used only to introspect end-user tokens. The service account needs the `view_user` permission granted to it in the Authentik admin UI (see `.env.example`) — there's no way to check that from code, so a missing grant surfaces as a 500 from `GET /users` (Authentik's API rejects the call). `AuthentikUserDirectory::discover` runs at boot next to `TokenIntrospection::discover` in `main.rs` and fails startup the same way (`AppError::Auth`) if the token is missing; a per-request failure is `AppError::Authentik` instead, kept distinct from `Auth` because `Auth` is documented as boot-time-only. Results are paged server-side (200/page) and service-account/inactive users are filtered out — assignment targets must be real people. Besides `id`/`name` the endpoint also passes through `email`, `avatarUrl` (Authentik's avatar: a URL or a `data:`-URI initials image), and the assigned role/group names, which the assignee picker and the Users table render.
